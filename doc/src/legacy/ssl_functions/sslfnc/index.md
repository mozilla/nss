(mozilla-projects-nss-ssl-functions-sslfnc)=
...
of NSS initialization functions, see [Initialization](sslintro.html#1027662).

Note that at least one of the functions listed in [SSL Export Policy Functions](#1098841) must
...
`` `NSS_InitReadWrite `` \<#1237143>\`\_\_ instead.
...
- `` `NSS_SetDomesticPolicy `` \<#1228530>\`\_\_, `` `NSS_SetExportPolicy `` \<#1100285>\`\_\_, and

> `` `NSS_SetFrancePolicy `` \<#1105952>\`\_\_ configure the cipher suites for domestic,
> international, and French versions of software products with encryption features.

- `` `SSL_CipherPolicySet `` \<#1104647>\`\_\_ sets policy flags for individual cipher suites, one at
...
Sets up configuration files and performs other tasks required to run Network Security Services.
Unlike `` `NSS_Init `` \<#1067601>\`\_\_, `NSS_InitReadWrite` provides both read and write access to
database files.
...
Use `NSS_InitReadWrite` rather than `` `NSS_Init `` \<#1067601>\`\_\_ if you are performing
...
`SSL_OptionSetDefault` replaces the deprecated function `` `SSL_EnableDefault `` \<#1206365>\`\_\_.
...
- If unsuccessful, `SECFailure`. Use
  `` `PR_GetError `` \<<https://developer.mozilla.org/en-US/docs/Mozilla/Projects/NSPR/Reference/PR_GetError>>\`\_\_
...
- As mentioned in [Communication](sslintro.html#1027816), when an application imports a
  ...
  the behavior of
  `` `PR_Connect `` \<<https://developer.mozilla.org/en-US/docs/Mozilla/Projects/NSPR/Reference/PR_Connect>>\`\_\_
  and
  `` `PR_Accept `` \<<https://developer.mozilla.org/en-US/docs/Mozilla/Projects/NSPR/Reference/PR_Accept>>\`\_\_
  ...
  `SSL_EnableDefault` and `SSL_Enable`, see [SSL_OptionSet](#1086543).
...
`` `SSL_OptionSetDefault `` \<#1068466>\`\_\_.
...
disallowed by previous calls to one or more of the [SSL Export Policy Functions](#1098841)).
...
for an individual socket, use `` `SSL_CipherPrefSet `` \<#1214758>\`\_\_.
...
enable or disable a cipher suite via calls to `` `SSL_CipherPrefSetDefault `` \<#1084747>\`\_\_, and
...
Functions](#1098841) for details. Your application must call one of the export policy
...
Functions](#1098841) to change cipher suite policy settings or use
`` `SSL_CipherPrefSetDefault `` \<#1084747>\`\_\_ to enable or disable any cipher suite. Otherwise, the
...
> shutdown, `` `NSS_Shutdown `` \<#1061858>\`\_\_ fails with the error code `SEC_ERROR_BUSY`.
...
multiple threads), use `` `SSL_ConfigMPServerSIDCache `` \<#1142625>\`\_\_ instead. You must use one of
...
`` `SSL_ConfigMPServerSIDCache `` \<#1142625>\`\_\_, and then creates child processes, by one of these
methods:
...
`` `SSL_ConfigMPServerSIDCache `` \<#1142625>\`\_\_ sets an environment variable named
...
In either case, the child must call `` `SSL_InheritMPServerSIDCache `` \<#1162055>\`\_\_ to complete
...
This function is identical to `` `SSL_ConfigServerSessionIDCache `` \<#1143851>\`\_\_, except that it
...
process must complete its initialization using `` `SSL_InheritMPServerSIDCache `` \<#1162055>\`\_\_.
...
you have used. (See [Initializing Multi-Processing with a Shared SSL Server Cache](#1154189).)
...
`` `SSL_CipherPrefSetDefault `` \<#1084747>\`\_\_; but bear in mind that
`` `SSL_CipherPrefSetDefault `` \<#1084747>\`\_\_ can't enable any cipher suite that is not explicitly
...
`` `NSS_SetDomesticPolicy `` \<#1228530>\`\_\_, `` `NSS_SetExportPolicy `` \<#1100285>\`\_\_, and
`` `NSS_SetFrancePolicy `` \<#1105952>\`\_\_ permit the use of approved cipher suites for domestic,
...
U.S. Export Regulations](http://w3.access.gpo.gov/bxa/ear/ear_data.html)) and French import
regulations.
...
use `` `SSL_CipherPolicySet `` \<#1104647>\`\_\_ to explicitly enable those cipher suites you may
legally export.
...
`` `SSL_CipherPolicyGet `` \<#1210463>\`\_\_
...
`` `SSL_CipherPolicySet `` \<#1104647>\`\_\_ for software that is*not* intended for export, and is
...
(but not necessarily enabled; see [SSL Export Policy Functions](#1098841)) for the calling
application.
...
first call `` `NSS_SetDomesticPolicy `` \<#1228530>\`\_\_, `` `NSS_SetExportPolicy `` \<#1100285>\`\_\_, or
`` `NSS_SetFrancePolicy `` \<#1105952>\`\_\_, then call `` `SSL_CipherPolicySet `` \<#1104647>\`\_\_
...
`` `SSL_CipherPolicySet `` \<#1104647>\`\_\_ to conform with current U.S. export regulations related to
...
[SSL Export Policy Functions](#1098841)). Policy for these suites is set to `SSL_ALLOWED`
...
`` `SSL_CipherPolicySet `` \<#1104647>\`\_\_.
...
you should first call `` `NSS_SetDomesticPolicy `` \<#1228530>\`\_\_,
`` `NSS_SetExportPolicy `` \<#1100285>\`\_\_, or `` `NSS_SetFrancePolicy `` \<#1105952>\`\_\_, then call
`` `SSL_CipherPolicySet `` \<#1104647>\`\_\_ repeatedly to explicitly allow or disallow cipher suites
...
`` `SSL_CipherPolicySet `` \<#1104647>\`\_\_ to conform with current U.S. export regulations and French
...
necessarily enable them; see [SSL Export Policy Functions](#1098841)). Policy for these suites
...
`SSL_CipherPolicySet` replaces the deprecated function `` `SSL_SetPolicy `` \<#1207350>\`\_\_.
...
`` `NSS_SetDomesticPolicy `` \<#1228530>\`\_\_, `` `NSS_SetExportPolicy `` \<#1100285>\`\_\_, or
`` `NSS_SetFrancePolicy `` \<#1105952>\`\_\_. In that case, first call
`` `NSS_SetDomesticPolicy `` \<#1228530>\`\_\_, `` `NSS_SetExportPolicy `` \<#1100285>\`\_\_, or
`` `NSS_SetFrancePolicy `` \<#1105952>\`\_\_, then call `SSL_CipherPolicySet` repeatedly to
...
`` `NSS_SetExportPolicy `` \<#1100285>\`\_\_ and `` `NSS_SetFrancePolicy `` \<#1105952>\`\_\_.) When an SSL
...
Export Policy Functions](#1098841).
...
`SSL_CipherPolicyGet` is the complementary function for `` `SSL_CipherPolicySet `` \<#1104647>\`\_\_.
...
See the description above for `` `SSL_CipherPolicySet `` \<#1104647>\`\_\_.
:::
...
For a complete list of configuration functions, see [Configuration](sslintro.html#1027742).

[SSL Configuration](#1090577)

[Callback Configuration](#1089578)
:::
...
`SSL_OptionSet` replaces the deprecated function `` `SSL_Enable `` \<#1220189>\`\_\_.
...
As mentioned in [Communication](sslintro.html#1027816), when an application imports a socket
into SSL after the TCP connection on that socket has already been established, it must call
[SSL_ResetHandshake](#1058001) to indicate whether the socket is for a client or server. At
...
See the description above for `` `SSL_OptionSet `` \<#1086543>\`\_\_.
...
the preference (which can be set with `` `SSL_CipherPrefSetDefault `` \<#1084747>\`\_\_). If an
...
`` `SSL_CipherPrefSetDefault `` \<#1084747>\`\_\_.
...
`` `NSS_FindCertKEAType `` \<sslcrt.html#1056950>\`\_\_. You can then pass the `SSLKEAType` value
...
use for them by calling `` `CERT_DestroyCertificate `` \<sslcrt.html#1050532>\`\_\_ and
`` `SECKEY_DestroyPrivateKey `` \<sslkey.html#1051017>\`\_\_.
...
function gets this string by calling `` `SSL_RevealURL `` \<#1081175>\`\_\_.
...
`` `PK11_SetPasswordFunc `` \<pkfnc.html#1023128>\`\_\_.
...
`` `PK11_SetPasswordFunc `` \<pkfnc.html#1023128>\`\_\_ that takes place during NSS initialization.
...
You can obtain the PIN argument by calling `` `SSL_RevealPinArg `` \<#1123385>\`\_\_.
:::
...
`` `PK11_SetPasswordFunc `` \<pkfnc.html#1023128>\`\_\_ to set up the password callback function during
NSS initialization.

For examples of the callback functions listed here, see [Chapter 2, "Getting Started With
SSL."](gtstd.html#1005439)
...
> `` `SSL_AuthCertificate `` \<#1088888>\`\_\_.
...
`` `SSL_PeerCertificate `` \<#1096168>\`\_\_.
...
`` `SSL_SetURL `` \<#1087792>\`\_\_. To obtain that domain name, the callback calls
`` `SSL_RevealURL `` \<#1081175>\`\_\_.
...
`` `SSL_SetPKCS11PinArg `` \<#1088040>\`\_\_ for details). To obtain the value that was set with
`` `SSL_SetPKCS11PinArg `` \<#1088040>\`\_\_, the callback calls `` `SSL_RevealPinArg `` \<#1123385>\`\_\_.
...
`` `SSL_BadCertHook `` \<#1088928>\`\_\_. A bad-certificate callback function gives the application the
...
from [Chapter 2, "Getting Started With SSL."](gtstd.html#1005439)
...
`` `SSL_AuthCertificateHook `` \<#1088805>\`\_\_ to specify its own certificate authentication callback
function.
...
- If the SSL socket has not had a domain name set (that is, if `` `SSL_SetURL `` \<#1087792>\`\_\_ has
...
`` `SSL_AuthCertificate `` \<#1088888>\`\_\_ callback function has failed. This callback function
...
bad-certificate callback function calls `` `SSL_PeerCertificate `` \<#1096168>\`\_\_. Since it is
...
The callback may need to call one or more PK11 functions to obtain the services of a PKCS #11
module. Some of the PK11 functions require a PIN argument (see
`` `SSL_SetPKCS11PinArg `` \<#1088040>\`\_\_ for details). To obtain the value previously passed, the
callback calls `` `SSL_RevealPinArg `` \<#1123385>\`\_\_
...
`` `NSS_GetClientAuthData `` \<#1106762>\`\_\_ for details. Unlike
`` `SSL_AuthCertificate `` \<#1088888>\`\_\_, `` `NSS_GetClientAuthData `` \<#1106762>\`\_\_ is not a default
...
Unlike `` `SSL_AuthCertificate `` \<#1088888>\`\_\_, `NSS_GetClientAuthData` is not a default
callback function. You must set it explicitly with `` `SSL_GetClientAuthDataHook `` \<#1126622>\`\_\_
...
Reference](../../../../../nspr/reference/html/index.html). For a complete list of
communication functions used by SSL-enabled applications, see
[Communication](sslintro.html#1027816).
...
- If `SSL_SECURITY` has not been enabled with a call to
  `` `SSL_OptionSetDefault `` \<#1068466>\`\_\_ or `` `SSL_OptionSet `` \<#1086543>\`\_\_, the function
  returns zero.
...
call to
`` `PR_Read `` \<<https://developer.mozilla.org/en-US/docs/Mozilla/Projects/NSPR/Reference/PR_Read>>\`\_\_
would block.
...
`` `PR_Free `` \<<https://developer.mozilla.org/en-US/docs/Mozilla/Projects/NSPR/Reference/PR_Free>>\`\_\_.

```{rubric} SSL_GetSessionID
:name: ssl_getsessionid
```

Returns a `` `SECItem `` \<ssltyp.html#1026076>\`\_\_ structure containing the SSL session ID
...
This function returns a `` `SECItem `` \<ssltyp.html#1026076>\`\_\_ structure containing the SSL
...
`` `SSL_ForceHandshake `` \<#1133431>\`\_\_. To set the peer ID, you use `SSL_SetSockPeerID`.
...
`` `SSL_ConfigServerSessionIDCache `` \<#1143851>\`\_\_.
:::
...
information about this argument, see `` `SSL_SetPKCS11PinArg `` \<#1088040>\`\_\_.
:::
...
- `PR_Connect`
- `PR_Accept`
- `` `SSL_ReHandshake `` \<#1232052>\`\_\_ (after the first handshake is finished)
- `` `SSL_ResetHandshake `` \<#1058001>\`\_\_ (for sockets that were connected or accepted prior to
...
`SSL_ReHandshake` replaces the deprecated function `` `SSL_RedoHandshake `` \<#1231825>\`\_\_.
...
- If successful, `SECSuccess`.
- If unsuccessful, `SECFailure`. Use
  `` `PR_GetError `` \<../../../../../nspr/reference/html/prerr.html#26127>\`\_\_ to obtain the error
  code.
...
Closes the key and certificate databases that were opened by `` `NSS_Init `` \<#1067601>\`\_\_.
...
doesn't call `` `SSL_ClearSessionCache `` \<#1138601>\`\_\_ first), `NSS_Shutdown` fails with the
...
`SSL_EnableDefault` has been replaced by `` `SSL_OptionSetDefault `` \<#1068466>\`\_\_ and works the
same way.
...
`SSL_Enable` has been replaced by `` `SSL_OptionSet `` \<#1086543>\`\_\_ and works the same way.
...
previous calls to one or more of the [SSL Export Policy Functions](#1098841)). This function
must be called once for each cipher you want to enable or disable.

`SSL_EnableCipher` has been replaced by `` `SSL_CipherPrefSetDefault `` \<#1084747>\`\_\_ and works
the same way.
...
`SSL_SetPolicy` has been replaced by `` `SSL_CipherPolicySet `` \<#1104647>\`\_\_ and works the same
way.
