(mozilla-projects-nss-ssl-functions-gtstd)=
...
[SSL, PKCS #11, and the Default Security Databases](#1011970)

[Setting Up the Certificate and Key Databases](#1011987)

[Building NSS Programs](#1013274)
:::
...
Security Services](../../intro.html). Before running the sample programs, it's important to
...
[Figure 2.1](#1013181) illustrates the relationships between NSPR, SSL, PKCS #11, and the
...
- The FIPS 140-1 module is compliant with the FIPS 140-1 government standard for implementations
  ...
  Certificate DB token (see [Figure 2.1](#1013181)), which handles both cryptographic
...
[Setting Up the CA DB and Certificate](#1012301)

[Setting Up the Server DB and Certificate](#1012351)

[Setting Up the Client DB and Certificate](#1012067)

[Verifying the Server and Client Certificates](#1012108)
...
>   [Dogtag](http://pki.fedoraproject.org/wiki/PKI_Main_Page) [Certificate
>   System](http://www.redhat.com/certificate_system/), to set up and operate your own CA
...
[Using the Certificate Database Tool](../../tools/certutil.html).
:::
...
2. Create the self-signed Root CA certificate, specifying the subject name for the certificate.
...
3. Extract the CA certificate from the CA's certificate database to a file.
...
2. Import the new CA certificate into the server's certificate database, and mark it trusted for
...
3. Create the server certificate request, specifying the subject name for the server certificate.
...
4. This step simulates the CA signing and issuing a new server certificate based on the server's
...
5. Import (Add) the new server certificate to the server's certificate database in the
...
2. Import the new CA certificate into the client's certificate database, and mark it trusted for
   issuing certificates for SSL client and server authentication.
...
4. This step simulates the CA signing and issuing a new client certificate based on the client's
   certificate request. The new cert is signed with the CA's private key, so this operation uses
...
5. Add the new client certificate to the client's certificate database in the `client_db`
